...
function ToDoItem({todo, done, id}:Todo) {
    const {onRemove, onToggle} = useItemButton(id);
    const parseId = id.toString();
    return (
        <Styled.ToDoItemWrapper id={parseId}>
            <Styled.ToDoItemCheckBox done={done} onClick={onToggle}><MdDone/></Styled.ToDoItemCheckBox>
            <Styled.ToDoItemText done={done}>{todo}</Styled.ToDoItemText>
            <Styled.ToDoItemRemoveBox onClick={onRemove}><MdDelete/></Styled.ToDoItemRemoveBox>
